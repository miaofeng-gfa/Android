...
public final class ProcessUtil {
    ...
    public static String getForegroundProcessName() {
        ...
        if (android.os.Build.VERSION.SDK_INT > android.os.Build.VERSION_CODES.LOLLIPOP) {
            PackageManager packageManager = ApplicationUtil.getApp().getPackageManager();
            Intent intent = new Intent(Settings.ACTION_USAGE_ACCESS_SETTINGS);
            List<ResolveInfo> list = packageManager.queryIntentActivities(intent, PackageManager.MATCH_DEFAULT_ONLY);
            System.out.println(list);
            if (list.size() > 0) {// 有"有权查看使用权限的应用"选项
                try {
                    ApplicationInfo info = packageManager.getApplicationInfo(ApplicationUtil.getApp().getPackageName(), 0);
                    AppOpsManager aom = (AppOpsManager) ApplicationUtil.getApp().getSystemService(Context.APP_OPS_SERVICE);
...
//                        LogUtil.d("getForegroundApp", "没有打开\"有权查看使用权限的应用\"选项");
                    ...
                    UsageStatsManager usageStatsManager = (UsageStatsManager) ApplicationUtil.getApp().getSystemService(Context.USAGE_STATS_SERVICE);
    ...
    public static Set<String> getAllBackgroundProcesses() {
        ActivityManager am = (ActivityManager) ApplicationUtil.getApp().getSystemService(Context.ACTIVITY_SERVICE);
