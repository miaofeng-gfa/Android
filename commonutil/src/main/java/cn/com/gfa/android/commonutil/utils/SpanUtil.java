...
public final class SpanUtil {
    ...
    class CustomIconMarginSpan implements LeadingMarginSpan, android.text.style.LineHeightSpan {
        ...
        private Bitmap uri2Bitmap(final Uri uri) {
            try {
                return MediaStore.Images.Media.getBitmap(ApplicationUtil.getApp().getContentResolver(), uri);
            } catch (IOException e) {
                e.printStackTrace();
        ...
        private Bitmap resource2Bitmap(final int resourceId) {
            Drawable drawable = ContextCompat.getDrawable(ApplicationUtil.getApp(), resourceId);
    ...
    class CustomImageSpan extends CustomDynamicDrawableSpan {
        ...
        private CustomImageSpan(final Bitmap b, final int verticalAlignment) {
            ...
            mDrawable = new BitmapDrawable(ApplicationUtil.getApp().getResources(), b);
        ...
        public Drawable getDrawable() {
            ...
            if (mDrawable != null) {
                ...
                Bitmap bitmap;
                try {
                    InputStream is = ApplicationUtil.getApp().getContentResolver().openInputStream(mContentUri);
                    bitmap = BitmapFactory.decodeStream(is);
                    drawable = new BitmapDrawable(ApplicationUtil.getApp().getResources(), bitmap);
                    drawable.setBounds(0, 0, drawable.getIntrinsicWidth(), drawable.getIntrinsicHeight());
                    if (is != null) {
                        is.close();
                    }
                ...
                try {
                    drawable = ContextCompat.getDrawable(ApplicationUtil.getApp(), mResourceId);
                    drawable.setBounds(0, 0, drawable.getIntrinsicWidth(), drawable.getIntrinsicHeight());
                } catch (Exception e) {
